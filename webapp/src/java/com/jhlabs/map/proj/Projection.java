...
public abstract class Projection implements Cloneable {
  ...
  private double totalFalseNorthing = 0;
	...
	/**
	 * Project a lat/long point (in degrees), producing a result in metres (x,y)
	 */
	public double[] transform( double lon, double lat ) {
                double lonRad = lon * DTR;
                double latRad = lat * DTR;
		if ( projectionLongitude != 0 )
			lonRad = MapMath.normalizeLongitude( lonRad-projectionLongitude );
		double[] xy = project(lonRad, latRad);
		xy[0] = totalScale * xy[0] + totalFalseEasting;
		xy[1] = totalScale * xy[1] + totalFalseNorthing;
		return xy;
	}
	 ...
	 */
	protected abstract double[] project(double lam, double phi);

	/**
	 * Inverse-project a point (in metres), producing a lat/long result in degrees
	 */
	public double[] inverseTransform(double x, double y) {
		x = (x - totalFalseEasting) / totalScale;
		y = (y - totalFalseNorthing) / totalScale;
		double[] lonLat = projectInverse(x, y);
		if (lonLat[0] < -Math.PI)
			lonLat[0] = -Math.PI;
		else if (lonLat[0] > Math.PI)
			lonLat[0] = Math.PI;
		if (projectionLongitude != 0)
			lonLat[0] = MapMath.normalizeLongitude(lonLat[0]+projectionLongitude);
		lonLat[0] *= RTD;
		lonLat[1] *= RTD;
		return lonLat;
	}
	 ...
	 */
	protected abstract double[] projectInverse(double x, double y);
